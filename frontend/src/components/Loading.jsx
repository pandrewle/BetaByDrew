...
const CountAnimation = ({ delay = 1500, isPending, handleCompletion }) => {
  const [count, setCount] = useState(0);
  const [intervalDuration, setIntervalDuration] = useState(50);
  const [animationStarted, setAnimationStarted] = useState(false);
  const completionHandledRef = useRef(false); // Add a ref to track if completion has been handled
      ...
      interval = setInterval(() => {
        if (isPending && count < 99) {
          ...
          if (count > 50 && count < 90) {
            setIntervalDuration((prevDuration) => prevDuration + 10);
          } else if (count >= 90 && count < 99) {
            setIntervalDuration((prevDuration) => prevDuration + 50);
          }
        } else if (!isPending && !completionHandledRef.current) {
          clearInterval(interval);
          setCount(100); // Ensure the count reaches 100 when isPending is false
          handleCompletion();
          completionHandledRef.current = true; // Set the ref to true after handling completion
        }
      }, intervalDuration);
    }

    return () => clearInterval(interval);
  }, [count, intervalDuration, animationStarted, isPending]);
